...
- [x] (2026-03-28 00:00Z) Completed the qmd SDK migration follow-up, removed the final legacy CLI compatibility layer, kept `@tobilu/qmd` optional, and verified the clean-break `0.4.0` state with lint, build, diagnostics, and deterministic tests.
- [x] (2026-03-28 23:20Z) Started Milestone 3 with additive layout scaffolding: added explicit path helpers for `memory_summary.md`, `sessions/`, `topics/`, `skills/`, `graph/`, `search/`, `dream/`, and `archive/`; seeded `memory_summary.md` lazily; and added deterministic layout tests covering fresh installs, legacy installs, and idempotence.
- [x] (2026-03-28 23:55Z) Started Milestone 4 with session-memory scaffolding: added deterministic session path helpers, session `meta.json`, numbered checkpoint stubs, and rolling `summary.md` generation; wired `session_start`, `session_before_compact`, and `session_shutdown` to create additive session artifacts while preserving current prompt injection and daily-log behavior.
